...
public class Server implements Comparable {
  ...
  /** initialize the ip adrress and default weight once start. then register itself to serverpool */
  Server() {
  ...
  /**
   * initialize the ip adrress and specify the weight of server once start then register itself to
   * serverpool
   *
   * @param weight
   */
  ...
  /** server start listening create an empty file using the ip address */
  ...
  public int compareTo(Object o) {
    ...
    if (server.getWeight() > this.weight) return -1;
    else if (server.getWeight() < this.weight) return 1;
